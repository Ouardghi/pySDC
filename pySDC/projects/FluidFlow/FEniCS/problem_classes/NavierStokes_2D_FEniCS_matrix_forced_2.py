...
class fenics_ConvDiff_mass_timebc(fenics_ConvDiff_mass):
    ...
    def NSsolve(self,u,t,dt):
        ...
        #
        L2 = -(1/dt)*df.div(u.values)*self.q*df.dx
        Rhs2 = df.assemble(L2)
        ...
        #
        L3 = df.dot(u.values,self.v)*df.dx - dt*df.dot(df.nabla_grad(p),self.v)*df.dx
        Rhs3 = df.assemble(L3)
    ...
    def WriteFiles(self,t):
        
        self.xdmffile_p.write_checkpoint(self.pn, "pn", t, df.XDMFFile.Encoding.HDF5, True)
        self.xdmffile_u.write_checkpoint(self.un.values,"un", t, df.XDMFFile.Encoding.HDF5, True)
