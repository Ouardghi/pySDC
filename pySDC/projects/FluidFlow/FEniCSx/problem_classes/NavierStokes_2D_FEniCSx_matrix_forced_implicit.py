...
class fenicsx_NSE_mass(Problem):
    ...
    def __init__(self, nelems=128, t0=0.0, family='CG', order=4, refinements=1, nu=0.1, ofiles = None, comm=MPI.COMM_WORLD):
        ...
        self._makeAttributeAndRegister(
            'nelems', 't0', 'family', 'order', 'refinements', 'nu', 'ofiles', localVars=locals(), readOnly=True)
        ...
        u_noslip.interpolate(lambda x : np.vstack((np.full(x.shape[1], 0.0, dtype=np.float64), 
        ...
        walls  = dfx.mesh.locate_entities_boundary(domain, fdim, lambda x: np.isclose(x[1], 0.0) | np.isclose(x[1], 0.41))
        cylin  = dfx.mesh.locate_entities_boundary(domain, fdim, lambda x: np.isclose((x[0]-0.2)**2 + (x[1]-0.2)**2, (0.05)**2))
        #
        ...
        self.solver.rtol = 1e-15
        self.solver.atol = 1e-15
    ...
    def source_term(self, t):
        ...
        g.interpolate(lambda x : np.vstack((np.full(x.shape[1], 0.0, dtype=np.float64), 
                                            np.full(x.shape[1], 0.0, dtype=np.float64))))
    ...
    def u_exact(self, t):
        ...
        w0.sub(0).interpolate(lambda x : np.vstack((np.full(x.shape[1], 0.0, dtype=np.float64), 
                                                    np.full(x.shape[1], 0.0, dtype=np.float64))))
