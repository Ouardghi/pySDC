...
class implicit_1st_order_mass(Sweeper):
    """
    ...
    def __init__(self, params):
        ...
        if 'space_comm' not in params.keys():
            params['space_comm'] = MPI.COMM_WORLD
            self.logger.debug(
                'Using MPI.COMM_WORLD for the space communicator because none was supplied in the params.'
            )
    ...
    def compute_residual(self, stage=None):
        ...
        for m in range(self.coll.num_nodes):
            ...
            res_global = self.space_comm.allgather(abs(res[m]))
            res_norm.append(max(res_global))
